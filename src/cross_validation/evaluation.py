...
PATH_TO_RESULTS = '../collaborative-filtering/results/model_based_svd/'
...
if __name__ == '__main__':
    ...
    if os.path.exists(PATH_TO_RESULTS):
        ...
        for index, filename in enumerate(os.listdir(PATH_TO_RESULTS)):
            ...
            part_dict = create_dict_from_part(parts[index])  
            if splited_type_algo[0] == "item" or splited_type_algo[0] == "user" :
                ...
                rmse = RMSE(error_values,len(parts[index]))
                mae = MAE(error_values,len(parts[index]))
                print(f"rmse:{rmse}")
                print(f"mae:{mae}")
            ...
            elif splited_type_algo[0] == "model":
                df = recommendation
                keys = part_dict.keys()
